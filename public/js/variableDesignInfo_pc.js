...
$(document).ready(function(){
    ...
    let layerText = '';
    let priceHtml = '';
    let nameHtml = designInfo.name;
    if(nameHtml === null || nameHtml === ''){
        nameHtml = '编号：'+designInfo.id;
        shareDescText += designInfo.type.nameCn;
    }else{
        shareDescText += designInfo.name + '，'+designInfo.type.nameCn;
    }

    shareDescText += layerText;
    let gender = ''
    if(designInfo.gender == 0){
        gender = '女款'
    }else if(designInfo.gender == 1){
        gender = '男款'
    }else if(designInfo.gender == 2){
        gender = '均适款'
    }

    shareDescText += '，'+gender

    let sc = '';

    if(designInfo.description){
        shareDescText += '，'+designInfo.description;
    }
	...
	if(userInfo.contactPhone){
		phone = '        <div class="row">电话：<a href="tel:'+userInfo.contactPhone+'">'+userInfo.contactPhone+'</a></div>\n'
	}
	if(userInfo.contactWxNo){
		wx = '        <div class="row">咨询微信：<div class="col-5"><input type="text" readonly id="wxNo" value="'+userInfo.contactWxNo+'"/></div><div class="col-3"></div></div>\n'
	}
    let infoHtml =  '<div>\n' +
        '        <div class="row">\n' +
    ...
    $('#bottom').html(infoHtml);
    if(designInfo.parts){
        let gemWeight = designInfo.parts[0].gemWeight
        if(gemWeight){
            $('.layInfo').html('<div class="row mgw">主石重量：'+gemWeight+'ct </div>\n')
        }else{
            $('.mgw').remove()
        }
    }
